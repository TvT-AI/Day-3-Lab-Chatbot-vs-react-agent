...
class GeminiProvider(LLMProvider):
    ...
    def generate(self, prompt: str, system_prompt: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Fallback to simulated response if quota/rate limit exceeded
            # This enables offline execution when API quota is exhausted
            content = self._simulate_response(prompt, system_prompt)
            # Standard simulated token counts
            prompt_toks = len(full_prompt.split()) + 150
            ...
            print(f"[Fallback] API error occurred (quota/auth issue). Falling back to Simulated LLM Generator...")
    ...
    def _simulate_response(self, prompt: str, system_prompt: Optional[str] = None) -> str:
        ...
        if not is_react:
            # Chatbot direct baseline responses (which confidently hallucinate or fail)
        ...
        # ReAct Agent loop step responses (vietnam traffic law database)
        if "tốc độ" in prompt_lower and "ô tô" in prompt_lower:
            ...
            # TC4: Ô tô nồng độ cồn 0.35 mg/l
            ...
            # TC2: Xe máy nồng độ cồn chưa vượt quá 50 mg/100ml
